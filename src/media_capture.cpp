    ...
    StgcnGpuOffload &stgcn_model)
{
  ...
  const CocoFrame norm_frame = normalize_coco_frame(raw_frame, frame_width, frame_height);

  const std::optional<std::vector<float>> window = window_buffer.add_frame(norm_frame);
